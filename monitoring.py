...
# Create metrics
metric_gauge = Gauge('custom_metric', 'A custom gauge metric')
metric_counter = Counter('custom_counter', 'A custom counter metric')
...
# =============================
# NEW: Device Availability Tracking for Drill-Down
# =============================

# Device availability over time (1=Online, 0=Offline)
device_availability = Gauge(
    'device_availability',
    'Device availability status (1=Online, 0=Offline)',
    ['mac_address', 'ip_address', 'hostname']
)

# Device uptime percentage (calculated)
device_uptime_percentage = Gauge(
    'device_uptime_percentage',
    'Device uptime percentage over the last 24 hours',
    ['mac_address', 'ip_address']
)

# Device response time (if pingable)
...
# Device state changes (for timeline visualization)
device_state_change = Counter(
    'device_state_change_total',
    'Number of times device changed state',
    ['mac_address', 'ip_address', 'old_state', 'new_state']
)
...
# =============================
# NEW: Active device availability check (ping-based)
# =============================

def ping_device(ip, mac, hostname):
    """Active ping check for device availability"""
    try:
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                if "time=" in output:
                    ...
                    device_response_time_ms.labels(mac_address=mac, ip_address=ip).set(response_time)
                    device_packet_loss_percent.labels(mac_address=mac, ip_address=ip).set(0)
                else:
                    device_response_time_ms.labels(mac_address=mac, ip_address=ip).set(0)
            except:
                pass
            
            # Device is online
            old_status = device_availability._value.get({
                'mac_address': mac, 
                'ip_address': ip, 
                'hostname': hostname
            })
            ...
            device_availability.labels(mac_address=mac, ip_address=ip, hostname=hostname).set(1)
            return True
        else:
            # Device is offline
            old_status = device_availability._value.get({
            ...
            device_availability.labels(mac_address=mac, ip_address=ip, hostname=hostname).set(0)
            device_packet_loss_percent.labels(mac_address=mac, ip_address=ip).set(100)
            return False
    except:
        # Timeout or error - device is offline
        device_availability.labels(mac_address=mac, ip_address=ip, hostname=hostname).set(0)
        device_packet_loss_percent.labels(mac_address=mac, ip_address=ip).set(100)
        return False

def active_device_monitor():
    """Periodically check device availability via ping"""
    while True:
        with lock:
            devices_to_check = []
            for mac, info in device_state.items():
                ip = info.get("ip", "unknown")
                hostname = info.get("hostname", mac.replace(":", "")[-8:])
                
                # Only check devices that are private IPs
                if ip != "unknown" and is_private_ip(ip):
                    devices_to_check.append((mac, ip, hostname))
        
        # Check each device
        for mac, ip, hostname in devices_to_check:
            try:
                ping_device(ip, mac, hostname)
            except Exception as e:
...
def calculate_uptime_percentage():
    """Calculate and update uptime percentage for devices"""
    while True:
        ...
        with lock:
            for mac, info in device_state.items():
                ...
                if ip != "unknown" and is_private_ip(ip):
                    # Query the availability metric over last 24 hours
                    # Note: This is a calculation that would normally be done in Prometheus
                    # Here we're just providing a placeholder
                    current_status = device_availability._value.get({
                        'mac_address': mac,
                        'ip_address': ip,
                        'hostname': hostname
                    })
                    ...
                    if not hasattr(calculate_uptime_percentage, "uptime_counts"):
                        calculate_uptime_percentage.uptime_counts = {}
                    
                    key = f"{mac}_{ip}"
                    if key not in calculate_uptime_percentage.uptime_counts:
                        calculate_uptime_percentage.uptime_counts[key] = {"online": 0, "total": 0}
                    
                    calculate_uptime_percentage.uptime_counts[key]["total"] += 1
                    if current_status == 1:
                        calculate_uptime_percentage.uptime_counts[key]["online"] += 1
                    
                    # Calculate percentage (over last 60 samples = last hour)
                    uptime = (calculate_uptime_percentage.uptime_counts[key]["online"] / 
                             max(1, calculate_uptime_percentage.uptime_counts[key]["total"])) * 100
                    
                    device_uptime_percentage.labels(mac_address=mac, ip_address=ip).set(uptime)
            ...
            for mac in stale_macs:
                ...
                for known_ip in known_ips:
                    try:
                        ...
                        device_availability.remove(mac_address=mac, ip_address=known_ip)
                    except Exception as e:
    ...
    while time.time() - start_time < ARP_CAPTURE_DURATION:
        try:
            ...
            if sender_mac not in device_state:
                ...
                hostname = f"device_{len(device_state) + 1}"
                ...
                # Initialize availability metric
                device_availability.labels(mac_address=sender_mac, ip_address=sender_ip, hostname=hostname).set(1)
            else:
                ...
                if sender_ip not in known_ips:
                    ...
                    device_availability.labels(
                        ...
                        hostname=device_state[sender_mac].get("hostname", sender_mac.replace(":", "")[-8:])
                    ).set(1)
    ...
    for mac, info in device_state.items():
        ...
        if now - info["last_seen"] > DEVICE_TIMEOUT:
            if info["online"] is True:
                ...
                # Mark device as offline in availability metric
                device_availability.labels(mac_address=mac, ip_address=ip, hostname=hostname).set(0)
            ...
            # Ensure availability metric is 1 for active devices
            device_availability.labels(mac_address=mac, ip_address=ip, hostname=hostname).set(1)
            active_count += 1
...
if __name__ == "__main__":
    ...
    # NEW: Active device availability monitor
    threading.Thread(target=active_device_monitor, daemon=True).start()
    
    # NEW: Uptime percentage calculator
    threading.Thread(target=calculate_uptime_percentage, daemon=True).start()
